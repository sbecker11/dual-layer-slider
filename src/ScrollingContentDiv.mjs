...
export class ScrollingContentDiv {
    ...
    constructor(htmlContentUrl, backgroundImageUrls, isSingleWindow = false) {
        ...
        this.content_item_height = null; // Height of a single content item (for calculations)
...
// deepcode ignore DOMXSS: because it's been sanitized using DOMPurify
    ...
    startAnimationLoop() {
        // Calculate content item height if not already calculated
        if (this.content_item_height === null && this.htmlContentDiv) {
            const firstItem = this.htmlContentDiv.querySelector('.image-card, [data-item], [class*="card"], [class*="item"]');
            if (firstItem) {
                this.content_item_height = firstItem.getBoundingClientRect().height;
                if (this.verbose) console.log('Content item height:', this.content_item_height, 'px');
            }
        }
    ...
    getSanitizedHtmlContent(html_content) {
        ...
        try {
            ...
            const config = {
                ...
                ALLOWED_ATTR: ['class', 'id', 'src', 'alt', 'loading', 'href', 'type', 'rel', 'width', 'height'],
